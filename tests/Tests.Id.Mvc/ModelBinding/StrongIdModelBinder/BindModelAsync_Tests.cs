...
namespace Wrap.Mvc.ModelBinding.StrongIdModelBinder_Tests;
...
public class BindModelAsync_Tests
{
		...
		where TIdValue : struct
	{
		...
		parse.Invoke(default).ReturnsForAnyArgs(M.None);

		var binder = Substitute.For<IdModelBinder<TId, TIdValue>>();
