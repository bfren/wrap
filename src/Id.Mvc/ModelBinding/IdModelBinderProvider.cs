...
using Wrap.Mvc.Exceptions;

namespace Wrap.Mvc.ModelBinding;

/// <summary>
/// <see cref="Id{TId, TValue}"/> MVC model binder provider.
/// </summary>
public sealed class IdModelBinderProvider : IModelBinderProvider
{
	/// <summary>
	/// If the model type implements <see cref="Id{TId, TValue}"/>, create <see cref="IdModelBinder{TId, TIdValue}"/>
	/// </summary>
	...
	internal static IModelBinder? GetBinderFromModelType(Type modelType)
	{
		...
		return Activator.CreateInstance(genericType) switch
		{
			...
			_ =>
				throw new ModelBinderException($"Unable to create {typeof(IdModelBinder<,>)} for type {modelType}.")
		};
	}
}
