...
public static partial class ResultExtensions
{
	...
	public static T Unwrap<T>(this Result<T> @this, Func<FailureValue, T> ifFailed) =>
		R.Match(@this,
			fail: ifFailed,
			ok: x => x
		);
	...
	public static T Unwrap<T>(this Result<T> @this, Action<FailureValue> ifFailed, Func<T> returnThis) =>
		R.Match(@this,
			fail: f => { ifFailed(f); return returnThis(); },
			ok: x => x
		);

	/// <inheritdoc cref="Unwrap{T}(Result{T}, Action{FailureValue}, Func{T})"/>
	public static Task<T> UnwrapAsync<T>(this Result<T> @this, Func<FailureValue, Task> ifFailed, Func<T> returnThis) =>
		R.MatchAsync(@this,
			fail: async f => { await ifFailed(f); return returnThis(); },
			ok: x => x
		);
	...
	public static Task<T> UnwrapAsync<T>(this Result<T> @this, Func<FailureValue, Task> ifFailed, Func<Task<T>> returnThis) =>
		R.MatchAsync(@this,
			fail: async f => { await ifFailed(f); return await returnThis(); },
			ok: x => x
		);
